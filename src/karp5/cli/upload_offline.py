...
def create_mode(alias, suffix, with_id=False):
    ...
    for index in to_create:
        ...
        try:
            ...
            _logger.error(
                "Any documentes uploaded to ES index %s are removed." % newname
            )
            _logger.error(
...
def add_lexicon(to_add_name, to_add_file, alias, suffix):
    ...
    try:
        ...
        print("If data was uploaded to SQL you will have to remove it manually.")
...
def copy_alias_to_new_index(
    ...
    es_source = es_dsl.Search(using=conf_mgr.elastic(source_mode), index=source_mode)
