# -*- coding:UTF-8 -*-
import converter.lmflib as xml
import json
import logging
...
def sentimentlex(obj, lmf):
    ...
    if sense_obj['SenseExamples'][0].get('text'):
        example = xml.SenseExample(sense_obj['SenseExamples'][0]['text'], _type="text")
        sense.add_sense_example(example)
    if sense_obj['SenseExamples'][0].get('lemgrams'):
        example = xml.SenseExample(sense_obj['SenseExamples'][0]['lemgrams'], _type="lemgrams")
        sense.add_sense_example(example)
...
def fsvm(obj, lmf):
    ...
    for form in obj['FormRepresentations']:
        fr = xml.FormRepresentation()
        fr.add_feature(xml.Feature("writtenForm", form['baseform']))
        add_features(form, fr, ["lemgram", "inherent", "paradigm", "partOfSpeech"])
        le.lemma.add_form_representation(fr)
    wflist = obj.get('WordForms', [])
    for wf in wflist:
        wordform = xml.WordForm()
...
def hellqvist(obj, lmf):
    ...
    for sense_obj in obj['Sense']:
        ...
        add_features(sense_obj, sense, ['styledText', 'hellqvistStatus', 'faksimilID'])
...
def swefn(obj, lmf):
    ...
    lemma = xml.Lemma()
    le.lemma = lemma
    single_features = ['BFNID', 'styledText', 'createdBy', 'entry_status', 'createdDate']
    multiple_features = ['LU', 'compound', 'compoundExample', 'coreElement', 'domain', 'SweCxnID', 'problematicLU',
                         'suggestionForLU', 'inheritance', 'semanticType', 'comment', 'peripheralElement',
                         'BFNLUs', 'internal_comment']
    for sense_obj in obj['Sense']:
        sense = xml.Sense(sense_obj.get('senseid', ''))
...
def schlyter(obj, lmf):
    #   u'..FormRepresentations.partOfSpeech', u'..FormRepresentations.baseform', u'..FormRepresentations.lemgram',
    #   u'..FormRepresentations.information' u'..FormRepresentations.hwtext' u'..FormRepresentations.gram',
    #   u'..FormRepresentations.variant'

    #   u'..Sense.gram', u'..Sense.V?U0A00', u'..Sense', , u'..Sense.V?IPA**', u'..Sense.V?ISA**',
    #   u'..Sense.definition.text', u'..Sense.senseid', , u'..Sense.text'

    #   u'..WordForms.msd', u'..WordForms', , u'..WordForms.writtenForm'
    #   u'..lexiconName'
    #   u'..lexiconOrder',
    #   u'..xml'
    ...
    le = xml.LexicalEntry()
    lemma = xml.Lemma()
    le.xml = obj['xml']  # remove later, keep for validation? Where to put it in the lmf?
    le.lemma = lemma
    for form in obj['FormRepresentations']:
        fr = xml.FormRepresentation()
...
def soederwall(obj, lmf):
    # '..FormRepresentations.partOfSpeech', u'..FormRepresentations.lemgram', u'..FormRepresentations.oldForm',
    # '..FormRepresentations.gram', u'..FormRepresentations.baseform', u'..FormRepresentations.information',
    # '..Sense.V0IPA**', u'..Sense.definition.text', u'..Sense', u'..Sense.writtenForm', u'..Sense.senseid',
    # '..Sense.gram', u'..Sense.V0ISA**', u'..Sense.text', u'..Sense.AF0USNI'
    # '..WordForms.tag', u'..WordForms.msd', u'..WordForms.writtenForm', u'..WordForms',
    # '..lexiconOrder', u'..lexiconName', u'..xml'
    le = xml.LexicalEntry()
    lemma = xml.Lemma()
    le.xml = obj['xml']  # remove later, keep for validation? Where to put it in the lmf?
    le.lemma = lemma
...
def soederwallsupp(obj, lmf):
    # u'..WordForms.tag', u'..WordForms', u'..WordForms.msd',
    # u'..WordForms.writtenForm',
    # u'..Sense.gram', u'..Sense', u'..Sense.writtenForm'
    # u'..Sense.senseid', u'..Sense.definition.text',
    # u'..Sense.text', u'..Sense.AF0USNI'
    # u'..FormRepresentations.partOfSpeech', u'..FormRepresentations.lemgram', u'..FormRepresentations.baseform',
    # u'..FormRepresentations.information', u'..FormRepresentations.gram',
    le = xml.LexicalEntry()
    lemma = xml.Lemma()
    le.xml = obj['xml']  # remove later, keep for validation? Where to put it in the lmf?
    le.lemma = lemma
...
def termswefin(obj, lmf):
    ...
    for ex in [obj.get('baselang')]+obj.get('targetlang'):
        ...
        if lang in examples:
            logging.warning('Error, to many representations of %s, %s' % (lang, ex['wordform']))
...
def mklmf(objects, lexfunc, lang='swe'):
    ...
    for obj in objects:
        try:
            if type(obj) != dict:
                try:
                    obj = json.loads(obj, encoding='utf8')
                except Exception as e:
                    raise e
            lexfunc(obj, lmf)
        except Exception as e:
            logging.error('obj %s' % obj)
            raise e
...
if __name__ == '__main__':
    ...
    #import codecs
    #mklmf(json.loads(codecs.open('data/bilingual/term-swefin.json', encoding='utf8').read()),
    #      termswefin)
