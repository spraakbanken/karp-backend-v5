...
from karp.dbhandler import dbhandler as db
from . import errorhandler as eh
from karp.sb import searching as sb_searching
from karp.server.auth import validate_user
from karp.server.helper import config_manager as configM
from karp.server.translator import fieldmapping as F
from karp.server.translator import parser
from karp.server.translator import parsererror as PErr

import logging
""" Methods for querying the data base """


def query(elastic, page=0):
    try:
        ans = requestquery(elastic, page=page)
        return jsonify(ans)
...
def requestquery(elastic, page=0):
    ...
    try:
        ...
        raise eh.KarpQueryError("Could not parse data", debug_msg=e,
                                query=query)
    mode = settings['mode']
    sort = sortorder(settings, mode, query)
    ...
    ans = parser.adapt_query(size, start, elastic(mode=mode), loads(elasticq),
    ...
    if settings.get('format', '') in ['app', 'tryck']:  # saol
        ...
        ss.format_posts(ans, elastic, mode, index, settings['format'])
...
def sortorder(settings, mode, query):
    if not settings.get('sort', ''):
        if parser.get_command(query) == "simple":
...
def querycount(elastic, page=0):
    ...
    try:
        ...
        settings = parser.make_settings(permitted, default)
        q_ans = requestquery(elastic, page=page)
...
def check_bucketsize(bucket_sizes, size, index, elastic):
    is_more = {}
    for sizebucket, bucketname in bucket_sizes:
        countans = elastic.search(index=index, body=loads(sizebucket),
                                  search_type="count")
...
def formatpost(elastic):
    ...
    if to_format in ['app', 'tryck']:
        ...
        if type(data) != list:
            data = [data]
            # TODO add children?
        chits = ss.search_children(data, elastic(mode=mode), index)
        wfdict = ss.get_linkedwfs(data, elastic(mode=mode), index)
...
def autocomplete(elastic):
    ...
    try:
        ...
        for q in qs:
            ...
            if mode == "external":
                exp = sb.searching.autocompletequery(mode, boost, q)
            else:
                exp = autocompletequery(mode, boost, q)
...
def get_context(lexicon, elastic):
    ...
    sortvalue = control_escape(origentry_sort)
    logging.debug('Orig entry escaped key %s' % sortvalue)
    ...
    parser.parse_ext('and|%s|gte|%s' % (sortfieldname, sortvalue),
                     exps, [], mode)
    elasticq_post = parser.search(exps, [], [])

    parser.parse_ext('and|%s|lte|%s' % (sortfieldname, sortvalue),
                     preexps, [], mode)
    elasticq_pre = parser.search(preexps, [], [])
...
def control_escape(s):
    ...
    return ''.join([c.encode('unicode_escape').replace('\\x', '\\u00')
