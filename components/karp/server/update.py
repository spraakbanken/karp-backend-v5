...
import logging
from karp.sb.autoupdates import auto_update, autoupdate_child
from karp.server.helper import config_manager as configM
from urllib.parse import parse_qs
from karp.server import auth as auth
from karp.server.helper import helpers as helpers
from karp.server.translator import validatejson as validate
...
def delete_entry(lexicon, _id, elastic, sql=False, live=True, suggestion=False):
    # delete by id
    try:
        query = request.query_string
        parsed = parse_qs(query)
        msg = parsed.get('message', ['removed'])[0]
        es, index, typ = helpers.get_update_index(lexicon, elastic,
...
def update_doc(lexicon, _id, elastic, data=None, live=True):
    ...
    auto_update(data_doc, lexicon, actiontype='update', user=user, date=date)
    try:
...
def add_multi_doc(lexicon, elastic, index=''):
    ...
    try:
        ...
        for doc in documents:
            ...
            date = datetime.datetime.now()
            auto_update(doc, lexicon, actiontype='add', user=user, date=date)
...
def add_doc(lexicon, elastic, index='', _id=None, suggestion=False, data=None,
            live=True):
    ...
    try:
        ...
        date = datetime.datetime.now()
        auto_update(data_doc, lexiconName, actiontype='add', user=user,
                    date=date)
...
def add_child(lexicon, elastic, parentid, suggestion=False):
    ...
    # add child to parent
    autoupdate_child(data_doc, parent, lexicon, user, date='')
    parent_doc = {"doc": parent, "user": user, "message": msg}
    logging.debug('save parent %s', parent_doc)
    logging.debug('save child %s', parent_doc)
    parres = update_doc(lexicon, parentid, elastic, data=parent_doc, live=False)
    # add child to lexicon
    childres = add_doc(lexicon, elastic, index=index, data=data, live=False)
