...
def test_scan():
    ...
    errors = []
    for ok, item in es_helpers.streaming_bulk(es, actions=updated_index, index=target_index):
        if not ok:
            failed += 1
            errors.append(item)
        else:
...
def test_dsl_scan():
    ...
    success, failed, total = 0, 0, 0
    errors = []
    for ok, item in es_helpers.streaming_bulk(es, actions=source_docs, index=target_index):
        if not ok:
            failed += 1
            errors.append(item)
        else:
