...
@pytest.mark.parametrize('query_string,n_hits',[
    (None, 6609),
    ...
    ('q=extended||and|pos|equals|Vb||not|baseform|startswith|ab&mode=panacea', 1612),
...
def test_statistics(client_w_panacea, query_string, n_hits):
    ...
    if n_hits > 0:
        assert len(result['aggregations']['q_statistics']['lexiconName']['buckets']) == 1
        assert result['aggregations']['q_statistics']['lexiconName']['buckets'][0]['doc_count'] == n_hits
        assert result['aggregations']['q_statistics']['lexiconName']['buckets'][0]['key'] == 'panacea'
    else:
        assert len(result['aggregations']['q_statistics']['lexiconName']['buckets']) == 0
